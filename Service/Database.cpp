...
void Database::dropTable()
{
    ...
    if (sqlite3_step(createStmt) != SQLITE_DONE) return;
    ...
    if (sqlite3_step(createStmt1) != SQLITE_DONE) return;
...
void Database::createTablesForInfo()
{
    ...
    { 
        ...
        if (sqlite3_step(insertStmt) != SQLITE_DONE) return;
    }      
    sqlite3_close(db);
    sqlite3* db1 = 0; 
    sqlite3_open(this->filePath, &db1);
...
size_t Database::check(std::string buffer, uint32_t offset) 
{
    size_t rowNumber = 0;
